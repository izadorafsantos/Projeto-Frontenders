...
let rgTeste = $('#rg-mask').blur(function () {

  let strRG = $("#rg-mask").val();
  if (strRG.length < 4) {

    alert("Quantidade de dígitos inválidos!");
  } 
...
$('#confirm').blur(function (senha, confirsenha) {
  ...
  var confirsenha= $('#confirm').val();
        if (senha != confirsenha) {
         $('#confirm').append ('A senha estão diferentes, corriga os campos!'),
         $('#confirm').css ('style', 'strong');
          return false;
      } 
});
